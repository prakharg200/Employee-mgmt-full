...
export default function ETable() {
    ...
    useEffect(()=>{
        ...
        const response = await fetch("http://localhost:4000/api/employee/education/")
    ...
    const handleSubmit = async (e) => {
        ...
        if (editIndex === null) {
            ...
            const response = await fetch("http://localhost:4000/api/employee/education/"+editIndex,{
            method: "PATCH",
            body: JSON.stringify(newData),
            headers: {
                "content-type": "application/json"
  ...
  const deleteTableRow = async (id) =>{
    const response = await fetch("http://localhost:4000/api/employee/education/"+id,{
      method:"DELETE"
    })
                            ...
                            return (
                                <tr className='box' key={record.id}>
                                    ...
                                    <td className='edu-action-main'>
                                      <BiEdit className='edu-edit edu-action' onClick={() => editTableRow(record._id,index)} />
                                      <MdDelete className='edu-edit edu-action' onClick={() => deleteTableRow(record._id)} />
